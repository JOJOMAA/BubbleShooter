...
Game::Game() : ball(25.f), secondBall(25.f), window(sf::VideoMode(1080, 960), "BubbleShooter") {
...
std::vector<std::vector<Ball*>> grid(8, std::vector<Ball*>(21, nullptr));

void Game::insertIntoGrid(Ball& ball) {
    // Bestimmen Sie die Grid-Koordinaten basierend auf der Position des Balls
    int gridX = static_cast<int>(ball.bubble.getPosition().x / (2 * ball.bubble.getRadius()));
    int gridY = static_cast<int>(ball.bubble.getPosition().y / (2 * ball.bubble.getRadius()));

    // Überprüfen Sie, ob die Grid-Koordinaten innerhalb der Grenzen des Grids liegen
    if (gridX >= 0 && gridX < grid[0].size() && gridY >= 0 && gridY < grid.size()) {
        // Überprüfen Sie, ob das Grid-Feld frei ist
        if (grid[gridY][gridX] == nullptr) {
            // Fügen Sie den Ball in das Grid ein
            grid[gridY][gridX] = &ball;
        } else {
            // Behandeln Sie den Fall, dass das Grid-Feld bereits belegt ist
            // (z.B. suchen Sie nach dem nächsten freien Feld)
        }
    } else {
        // Behandeln Sie den Fall, dass die Grid-Koordinaten außerhalb der Grenzen des Grids liegen
        // (z.B. werfen Sie einen Fehler oder passen Sie die Koordinaten an)
    }
}
